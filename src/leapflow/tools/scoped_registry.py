...
class ScopedToolRegistry:
    ...
    def reload(self, plugin_id: str) -> PluginFiber:
        ...
        # 4. Since the registry is already assembled, assemble() will not re-run.
        #    Manually populate handlers/definitions/metadata for the new plugin —
        #    this mirrors the late-tool pattern.
        for tool in fresh_plugin.tools:
            self._registry._tool_handlers[tool.name] = tool.handler
            self._registry._tool_handlers[f"gp_{tool.name}"] = tool.handler
            self._registry._tool_definitions.append(tool.to_openai_schema())
            self._registry._all_metadata.append(tool)
        ...
        # 6. Bump the registry version so consumers (e.g. engine _registry_cache) invalidate.
